...
impl BackgroundStarField {
	pub fn new<'sdl_all, 'l>(game_services: &mut GameServices<'sdl_all, 'l>) -> Self {
		...
		BackgroundStarField {
			...
			scroll_start_ms: common::current_time_ms()
		}
	}

	fn show_layer<'sdl_all, 'l>(&self, texture_index: &i64, layer_index: usize, z: i64, game_services: &mut GameServices<'sdl_all, 'l>) {
		...
		for tile_width_index in 0..num_background_width {
			for tile_height_index in -num_background_height..num_background_height {
				...
				let scroll_offset_y = (((common::current_time_ms() - self.scroll_start_ms) * (layer_index + 1) as u64) as f64 / ((SCROLL_SPEED_MS as u32) as f64)) as i32;
				...
				background_tile.y = base_y + (scroll_offset_y % game_services.draw_context.screen_height() as i32) + layer_offset_y;
	...
	pub fn update<'sdl_all, 'l>(&mut self, game_services: &mut GameServices<'sdl_all, 'l>) {
		for (index, texture_index) in self.texture_index_layers.iter().enumerate() {
			self.show_layer(texture_index, index, BACKGROUND_Z_INDEX + index as i64, game_services);
		}
	}
}
